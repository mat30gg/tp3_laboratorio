 ...
 *
 */
int controller_loadFromBinary(char* path , LinkedList* pArrayListEmployee, int contador)
{
	int exito;
	exito = 0;
	FILE* pArchivo;
	...
	if(pArchivo!=NULL)
	{
		exito = parser_EmployeeFromBinary(pArchivo, pArrayListEmployee, contador);
		if(exito == 0)
		{
			printf("ERROR al abrir archivo");
		}
...
int controller_sortEmployee(LinkedList* pArrayListEmployee)
{
	...
	if(pArrayListEmployee != NULL)
	{
		printf("\nCon que criterio se ordena la lista:");
		printf("\n|| 1. Por nombre.");
		printf("\n|| 2. Por sueldo.");
		printf("\n|| :=: ");
		scanf("%d", &criterio);
...
int controller_saveAsText(char* path , LinkedList* pArrayListEmployee)
{
	...
	len = ll_len(pArrayListEmployee);
	printf("longitud de linked list: %d\n", len);
