...
void startLastId(char* dir, int* lastId)
{
	int id;
	FILE* aUltimo;

	aUltimo = fopen("ultimoId.bin", "rb+"); //Con lectura rb+ retorna NULL si no existe el archivo
	if(dir != NULL)
	{
		if(aUltimo == NULL)
		{
		...
		{
			id = fread(lastId, sizeof(int), 1, aUltimo);
		}
	}
	fclose(aUltimo);
}
...
int countEmployee(char* dir)
{
	...
	if(arch != NULL)
	{
		while(!feof(arch))
		{
			fread(aux, sizeof(Employee), 1, arch);
			cont++;
		}
	}
	fclose(arch);
...
int ordenNombre(void* e1, void* e2)
{
	...
	char nombre_e2[128];
	int returnAux;
	employee_getNombre(e1, nombre_e1);
	employee_getNombre(e2, nombre_e2);
	returnAux = strcmp(nombre_e1, nombre_e2);
	return returnAux;
}
...
int ordenSueldo(void* e1, void* e2)
{
	int sueldo_e1;
	int sueldo_e2;
	employee_getSueldo(e1, &sueldo_e1);
	employee_getSueldo(e2, &sueldo_e2);
	int returnAux;
	if(sueldo_e1 == sueldo_e2)
	{
		returnAux = 0;
	}
	else
